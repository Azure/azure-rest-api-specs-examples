...
func ExampleRunsClient_NewListPager() {
	...
	pager := clientFactory.NewRunsClient().NewListPager("myResourceGroup", "myRegistry", &armcontainerregistrytasks.RunsClientListOptions{
		Filter: to.Ptr(""),
		Top:    to.Ptr[int32](10)})
	for pager.More() {
		...
		// page = armcontainerregistrytasks.RunsClientListResponse{
		// 	RunListResult: armcontainerregistrytasks.RunListResult{
		// 		Value: []*armcontainerregistrytasks.Run{
		// 			{
		...
		// 					},
		// 					OutputImages: []*armcontainerregistrytasks.ImageDescriptor{
		// 						{
		// 							Digest: to.Ptr("sha256:cbbf2f9a99b47fc460d422812b6a5adff7dfee951d8fa2e4a98caa0"),
