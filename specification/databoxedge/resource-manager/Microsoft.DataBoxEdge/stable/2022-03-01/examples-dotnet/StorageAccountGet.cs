...
NullableResponse<DataBoxEdgeStorageAccountResource> response = await collection.GetIfExistsAsync(storageAccountName);
DataBoxEdgeStorageAccountResource result = response.HasValue ? response.Value : null;
