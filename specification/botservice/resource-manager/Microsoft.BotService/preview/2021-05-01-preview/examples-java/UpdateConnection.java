...
public final class Main {
    ...
    public static void updateConnectionSetting(com.azure.resourcemanager.botservice.BotServiceManager manager) {
        ConnectionSetting resource = manager.botConnections().getWithResponse("OneResourceGroupName", "samplebotname",
            "sampleConnection", com.azure.core.util.Context.NONE).getValue();
        resource.update()
            .withProperties(new ConnectionSettingProperties().withClientId("sampleclientid")
                .withClientSecret("fakeTokenPlaceholder").withScopes("samplescope")
                .withServiceProviderId("serviceproviderid").withServiceProviderDisplayName("serviceProviderDisplayName")
                .withParameters(
                    Arrays.asList(new ConnectionSettingParameter().withKey("fakeTokenPlaceholder").withValue("value1"),
