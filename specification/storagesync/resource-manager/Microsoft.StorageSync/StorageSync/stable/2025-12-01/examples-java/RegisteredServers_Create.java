
/**
 * Samples for RegisteredServers Create.
 */
public final class Main {
    /*
     * x-ms-original-file: 2025-12-01/RegisteredServers_Create.json
     */
    /**
     * Sample code: RegisteredServers_Create.
     * 
     * @param manager Entry point to StorageSyncManager.
     */
    public static void registeredServersCreate(com.azure.resourcemanager.storagesync.StorageSyncManager manager) {
        manager.registeredServers().define("080d4133-bdb5-40a0-96a0-71a6057bfe9a")
            .withExistingStorageSyncService("SampleResourceGroup_1", "SampleStorageSyncService_1")
            .withServerCertificate(
                "")
            .withAgentVersion("1.0.277.0").withServerOSVersion("10.0.14393.0").withServerRole("Standalone")
            .withServerId("080d4133-bdb5-40a0-96a0-71a6057bfe9a").withFriendlyName("afscv-2304-139")
            .withApplicationId("00000000-0000-0000-0000-000000000001").withIdentity(true).create();
    }
}
