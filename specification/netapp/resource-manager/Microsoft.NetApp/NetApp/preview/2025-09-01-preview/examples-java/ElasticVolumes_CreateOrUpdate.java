...
public final class Main {
    ...
    public static void elasticVolumesCreateOrUpdate(com.azure.resourcemanager.netapp.NetAppFilesManager manager) {
        manager.elasticVolumes().define("volume1").withRegion("eastus")
            .withExistingElasticCapacityPool("myRG", "account1", "pool1")
            .withProperties(new ElasticVolumeProperties().withFilePath("my-unique-file-path").withSize(107374182400L)
                .withExportPolicy(new ElasticExportPolicy().withRules(Arrays.asList(
                    new ElasticExportPolicyRule().withRuleIndex(1).withUnixAccessRule(ElasticUnixAccessRule.READ_ONLY)
                        .withNfsv3(ElasticNfsv3Access.ENABLED).withNfsv4(ElasticNfsv4Access.DISABLED)
                        .withAllowedClients(Arrays.asList("0.0.0.0/0")).withRootAccess(ElasticRootAccess.DISABLED))))
