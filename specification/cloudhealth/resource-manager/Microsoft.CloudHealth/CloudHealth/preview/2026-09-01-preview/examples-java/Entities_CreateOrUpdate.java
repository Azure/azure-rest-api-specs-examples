            ...
            .withProperties(new EntityProperties().withDisplayName("Orders API")
                ...
                .withSignalGroups(new SignalGroups().withAzureResource(new AzureResourceSignals()
                    ...
                    .withDependencies(new DependenciesSignalGroupV2().withAggregationType(AggregationType.MIN_HEALTHY)
                        ...
                        .withUnit(AggregationUnit.PERCENTAGE).withIgnoreUnknown(true)))
                .withSignalAggregationGroups(Arrays.asList(
                    new SignalAggregationGroup().withName("latency-and-errors").withDisplayName("Latency and errors")
                        .withAggregationType(AggregationType.WORST_OF)
                        .withMembers(Arrays.asList("error-rate", "p95-latency")),
                    new SignalAggregationGroup().withName("compute-utilization").withDisplayName("Compute utilization")
                        .withAggregationType(AggregationType.MIN_HEALTHY)
                        .withMembers(Arrays.asList("node-cpu", "pod-cpu")).withUnhealthyThreshold(50.0D)
                        .withUnit(AggregationUnit.PERCENTAGE).withIgnoreUnknown(true)))
