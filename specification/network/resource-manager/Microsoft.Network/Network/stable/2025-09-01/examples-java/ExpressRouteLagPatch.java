
import com.azure.resourcemanager.network.models.ExpressRouteLagUpdateTagsOrIdentityRequest;
import com.azure.resourcemanager.network.models.ManagedServiceIdentity;
import com.azure.resourcemanager.network.models.ManagedServiceIdentityUserAssignedIdentities;
import com.azure.resourcemanager.network.models.ResourceIdentityType;
import java.util.HashMap;
import java.util.Map;

/**
 * Samples for ExpressRouteLags Update.
 */
public final class Main {
    /*
     * x-ms-original-file: 2025-09-01/ExpressRouteLagPatch.json
     */
    /**
     * Sample code: Update express route lag tags or identity.
     * 
     * @param manager Entry point to NetworkManager.
     */
    public static void updateExpressRouteLagTagsOrIdentity(com.azure.resourcemanager.network.NetworkManager manager) {
        manager.serviceClient().getExpressRouteLags().updateWithResponse("rg1", "lagName",
            new ExpressRouteLagUpdateTagsOrIdentityRequest().withIdentity(new ManagedServiceIdentity()
                .withType(ResourceIdentityType.USER_ASSIGNED)
                .withUserAssignedIdentities(mapOf(
                    "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/rg1/providers/Microsoft.ManagedIdentity/userAssignedIdentities/lagidentity",
                    new ManagedServiceIdentityUserAssignedIdentities())))
                .withTags(mapOf("tag1", "value1", "tag2", "value2")),
            com.azure.core.util.Context.NONE);
    }

    // Use "Map.of" if available
    @SuppressWarnings("unchecked")
    private static <T> Map<String, T> mapOf(Object... inputs) {
        Map<String, T> map = new HashMap<>();
        for (int i = 0; i < inputs.length; i += 2) {
            String key = (String) inputs[i];
            T value = (T) inputs[i + 1];
            map.put(key, value);
        }
        return map;
    }
}
