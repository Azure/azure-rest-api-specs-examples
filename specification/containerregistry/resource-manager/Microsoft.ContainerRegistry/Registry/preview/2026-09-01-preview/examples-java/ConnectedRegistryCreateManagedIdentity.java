...
public final class Main {
    ...
    public static void connectedRegistryCreateManagedIdentity(
        ...
        manager.serviceClient().getConnectedRegistries().create("myResourceGroup", "myRegistry", "myConnectedRegistry",
            new ConnectedRegistryInner().withIdentity(new ManagedServiceIdentity()
                ...
                .withParent(new ParentProperties().withSyncProperties(
                    new SyncProperties().withSchedule("0 9 * * *").withSyncWindow(Duration.parse("PT3H"))
                        .withMessageTtl(Duration.parse("P2D")).withAuthType(AuthType.MANAGED_IDENTITY)))
