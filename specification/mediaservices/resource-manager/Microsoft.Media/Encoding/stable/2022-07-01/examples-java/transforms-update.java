...
public final class Main {
    ...
    public static void updateATransform(com.azure.resourcemanager.mediaservices.MediaServicesManager manager) {
        Transform resource = manager.transforms()
            ...
            .getValue();
        resource.update().withDescription("Example transform to illustrate update.")
            .withOutputs(Arrays.asList(new TransformOutput().withRelativePriority(Priority.HIGH).withPreset(
                new BuiltInStandardEncoderPreset().withPresetName(EncoderNamedPreset.H264MULTIPLE_BITRATE720P))))
            .apply();
    }
}
