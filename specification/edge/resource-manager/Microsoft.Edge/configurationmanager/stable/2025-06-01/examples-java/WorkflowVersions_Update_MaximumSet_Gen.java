...
public final class Main {
    ...
    public static void workflowVersionsUpdateMaximumSet(
        ...
        WorkflowVersion resource = manager.workflowVersions().getWithResponse("rgconfigurationmanager", "testname",
            "testname", "testname", com.azure.core.util.Context.NONE).getValue();
        resource.update().withProperties(new WorkflowVersionProperties().withStageSpec(Arrays.asList(new StageSpec()
            .withName("amrbjd").withSpecification(mapOf())
            .withTasks(Arrays.asList(new TaskSpec().withName("xxmeyvmgydbcwxqwjhadjxjod").withTargetId(
                "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/{resourceProviderNamespace}/{resourceType}/{resourceName}")
                .withSpecification(mapOf())))
            .withTaskOption(new TaskOption().withConcurrency(3).withErrorAction(
                new ErrorAction().withMode(ErrorActionMode.STOP_ON_ANY_FAILURE).withMaxToleratedFailures(0)))))
            .withSpecification(mapOf())).apply();
    }
