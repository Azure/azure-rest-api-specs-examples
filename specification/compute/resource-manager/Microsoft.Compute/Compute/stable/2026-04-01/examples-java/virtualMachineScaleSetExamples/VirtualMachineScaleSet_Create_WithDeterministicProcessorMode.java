...
public final class Main {
    /*
     * x-ms-original-file:
     * 2026-04-01/virtualMachineScaleSetExamples/VirtualMachineScaleSet_Create_WithDeterministicProcessorMode.json
     */
    /**
     * Sample code: Create a VMSS with Deterministic Processor Mode.
     * 
     * @param manager Entry point to ComputeManager.
     */
    public static void
        createAVMSSWithDeterministicProcessorMode(com.azure.resourcemanager.compute.ComputeManager manager) {
        manager.serviceClient().getVirtualMachineScaleSets().createOrUpdate("myResourceGroup", "{vmss-name}",
            new VirtualMachineScaleSetInner().withLocation("westus")
                .withSku(new Sku().withName("Standard_E2pds_v8").withTier("Standard").withCapacity(3L))
                ...
                .withVirtualMachineProfile(new VirtualMachineScaleSetVMProfile()
                    ...
                    .withHardwareProfile(
                        new VirtualMachineScaleSetHardwareProfile().withProcessorMode(ProcessorMode.DETERMINISTIC)))
