    ...
    public static void
        ...
        manager.containerApps().define("testcontainerApp0").withRegion("East US").withExistingResourceGroup("rg")
            ...
            .withConfiguration(new Configuration().withIngress(new Ingress().withExternal(true).withTargetPort(3000)
                ...
                .withDapr(new Dapr()
                    .withEnabled(true).withAppProtocol(AppProtocol.HTTP).withAppPort(3000).withHttpReadBufferSize(30)
                    .withHttpMaxRequestSize(10).withLogLevel(LogLevel.DEBUG).withEnableApiLogging(true))
                .withMaxInactiveRevisions(10).withRevisionTransitionThreshold(100).withService(
                    new Service().withType("redis")))
            .withTemplate(
                new Template()
                    ...
                    .withContainers(Arrays.asList(
                        new Container().withImage("").withImageType(ImageType.CLOUD_BUILD).withName("testcontainerApp0")
                            .withVolumeMounts(Arrays.asList(new VolumeMount().withVolumeName("azurefile").withMountPath(
                                "/mnt/path1").withSubPath("subPath1"), new VolumeMount().withVolumeName("nfsazurefile")
                                    .withMountPath("/mnt/path2").withSubPath("subPath2")))
                            .withProbes(Arrays.asList(new ContainerAppProbe().withHttpGet(new ContainerAppProbeHttpGet()
                                .withHttpHeaders(Arrays.asList(new ContainerAppProbeHttpGetHttpHeadersItem()
                                    .withName("Custom-Header").withValue("Awesome")))
                                .withPath("/health").withPort(8080)).withInitialDelaySeconds(3).withPeriodSeconds(3)
                                .withType(Type.LIVENESS)))))
                    .withScale(new Scale().withMinReplicas(1).withMaxReplicas(5).withCooldownPeriod(350)
                    ...
                    .withVolumes(Arrays.asList(new Volume().withName(
                        "azurefile").withStorageType(StorageType.AZURE_FILE).withStorageName("storage"), new Volume()
                            .withName(
                                "nfsazurefile")
                            .withStorageType(StorageType.NFS_AZURE_FILE).withStorageName("nfsStorage")))
