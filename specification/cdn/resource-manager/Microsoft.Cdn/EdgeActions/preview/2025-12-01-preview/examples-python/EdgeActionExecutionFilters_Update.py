from azure.identity import DefaultAzureCredential

from azure.mgmt.edgeactions import EdgeActionsMgmtClient

"""
# PREREQUISITES
    pip install azure-identity
    pip install azure-mgmt-edgeactions
# USAGE
    python edge_action_execution_filters_update.py

    Before run the sample, please set the values of the client ID, tenant ID and client secret
    of the AAD application as environment variables: AZURE_CLIENT_ID, AZURE_TENANT_ID,
    AZURE_CLIENT_SECRET. For more info about how to get the value, please see:
    https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal
"""


def main():
    client = EdgeActionsMgmtClient(
        credential=DefaultAzureCredential(),
        subscription_id="SUBSCRIPTION_ID",
    )

    response = client.edge_action_execution_filters.begin_update(
        resource_group_name="testrg",
        edge_action_name="edgeAction1",
        execution_filter="executionFilter1",
        properties={"properties": {"executionFilterIdentifierHeaderValue": "header-value2"}},
    ).result()
    print(response)


# x-ms-original-file: 2025-12-01-preview/EdgeActionExecutionFilters_Update.json
if __name__ == "__main__":
    main()
