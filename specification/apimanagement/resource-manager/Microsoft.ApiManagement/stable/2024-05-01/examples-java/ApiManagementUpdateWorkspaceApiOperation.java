    ...
    public static void
        ...
        manager.workspaceApiOperations()
            .updateWithResponse("rg1", "apimService1", "wks1", "echo-api", "operationId", "*",
                new OperationUpdateContract().withDisplayName("Retrieve resource").withMethod("GET")
                    .withUrlTemplate("/resource").withTemplateParameters(Arrays.asList())
                    .withRequest(new RequestContract().withQueryParameters(Arrays.asList(new ParameterContract()
                        ...
                        .withType("string").withDefaultValue("sample").withRequired(true)
                        .withValues(Arrays.asList("sample")))))
                    .withResponses(Arrays.asList(
                        new ResponseContract().withStatusCode(200).withDescription("Returned in all cases.")
                            .withRepresentations(Arrays.asList()).withHeaders(Arrays.asList()),
                        new ResponseContract().withStatusCode(500).withDescription("Server Error.")
                            .withRepresentations(Arrays.asList()).withHeaders(Arrays.asList()))),
