...
public final class Main {
    ...
    public static void createUpdateMachine(com.azure.resourcemanager.containerservice.ContainerServiceManager manager) {
        manager.serviceClient().getMachines().createOrUpdate("rg1", "clustername1", "agentpool1", "machine1",
            new MachineInner().withProperties(
                new MachineProperties().withHardware(new MachineHardwareProfile().withVmSize("Standard_DS1_v2"))
                    .withOperatingSystem(
                    ...
                    .withKubernetes(new MachineKubernetesProfile().withNodeLabels(mapOf("key1", "fakeTokenPlaceholder"))
                        .withOrchestratorVersion("1.30").withKubeletDiskType(KubeletDiskType.OS)
                        .withNodeTaints(Arrays.asList("Key1=Value1:NoSchedule")).withMaxPods(110))
                    .withMode(AgentPoolMode.USER).withPriority(ScaleSetPriority.SPOT).withTags(mapOf("name1", "val1"))),
