    ...
    public static void storageSyncServicesUpdate(com.azure.resourcemanager.storagesync.StorageSyncManager manager) {
        StorageSyncService resource
            = manager.storageSyncServices().getByResourceGroupWithResponse("SampleResourceGroup_1",
                "SampleStorageSyncService_1", com.azure.core.util.Context.NONE).getValue();
        resource.update().withTags(mapOf("Dept", "IT", "Environment", "Test"))
            .withIncomingTrafficPolicy(IncomingTrafficPolicy.ALLOW_ALL_TRAFFIC).withUseIdentity(true).apply();
    }
