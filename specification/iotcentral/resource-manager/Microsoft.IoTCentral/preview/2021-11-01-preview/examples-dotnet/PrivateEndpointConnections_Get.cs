...
NullableResponse<IotCentralPrivateEndpointConnectionResource> response = await collection.GetIfExistsAsync(privateEndpointConnectionName);
IotCentralPrivateEndpointConnectionResource result = response.HasValue ? response.Value : null;
