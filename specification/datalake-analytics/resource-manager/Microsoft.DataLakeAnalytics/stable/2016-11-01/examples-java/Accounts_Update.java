...
public final class Main {
    ...
    public static void
        ...
        DataLakeAnalyticsAccount resource = manager.accounts()
            .getByResourceGroupWithResponse("contosorg", "contosoadla", com.azure.core.util.Context.NONE).getValue();
        resource.update().withTags(mapOf("test_key", "fakeTokenPlaceholder"))
            .withComputePoliciesForUpdate(Arrays.asList(new UpdateComputePolicyWithAccountParameters()
                .withName("test_policy").withObjectId(UUID.fromString("34adfa4f-cedf-4dc0-ba29-b6d1a69ab345"))
                .withObjectType(AadObjectType.USER).withMaxDegreeOfParallelismPerJob(1).withMinPriorityPerJob(1)))
            .withFirewallRulesForUpdate(Arrays.asList(new UpdateFirewallRuleWithAccountParameters()
                .withName("test_rule").withStartIpAddress("1.1.1.1").withEndIpAddress("2.2.2.2")))
            .withFirewallState(FirewallState.ENABLED).withFirewallAllowAzureIps(FirewallAllowAzureIpsState.ENABLED)
            .withNewTier(TierType.CONSUMPTION).withMaxJobCount(1).withMaxDegreeOfParallelism(1)
            .withMaxDegreeOfParallelismPerJob(1).withMinPriorityPerJob(1).withQueryStoreRetention(1).apply();
    }
