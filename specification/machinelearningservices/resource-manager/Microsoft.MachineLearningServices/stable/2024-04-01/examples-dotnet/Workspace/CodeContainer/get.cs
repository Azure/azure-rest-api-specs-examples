...
NullableResponse<MachineLearningCodeContainerResource> response = await collection.GetIfExistsAsync(name);
MachineLearningCodeContainerResource result = response.HasValue ? response.Value : null;
