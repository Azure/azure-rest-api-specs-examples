
import com.azure.resourcemanager.cosmos.fluent.models.FleetResourceInner;
import java.util.HashMap;
import java.util.Map;

/**
 * Samples for Fleet Create.
 */
public final class Main {
    /*
     * x-ms-original-file: 2026-03-15/fleet/CosmosDBFleetCreate.json
     */
    /**
     * Sample code: CosmosDB Fleet Create.
     * 
     * @param manager Entry point to CosmosManager.
     */
    public static void cosmosDBFleetCreate(com.azure.resourcemanager.cosmos.CosmosManager manager) {
        manager.serviceClient().getFleets().createWithResponse("rg1", "fleet1", new FleetResourceInner()
            .withLocation("West US").withTags(mapOf("Dept", "Finance", "Environment", "Production")),
            com.azure.core.util.Context.NONE);
    }

    // Use "Map.of" if available
    @SuppressWarnings("unchecked")
    private static <T> Map<String, T> mapOf(Object... inputs) {
        Map<String, T> map = new HashMap<>();
        for (int i = 0; i < inputs.length; i += 2) {
            String key = (String) inputs[i];
            T value = (T) inputs[i + 1];
            map.put(key, value);
        }
        return map;
    }
}
