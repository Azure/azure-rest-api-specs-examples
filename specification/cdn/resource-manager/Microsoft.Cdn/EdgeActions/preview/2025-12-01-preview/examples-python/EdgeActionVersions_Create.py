from azure.identity import DefaultAzureCredential

from azure.mgmt.edgeactions import EdgeActionsMgmtClient

"""
# PREREQUISITES
    pip install azure-identity
    pip install azure-mgmt-edgeactions
# USAGE
    python edge_action_versions_create.py

    Before run the sample, please set the values of the client ID, tenant ID and client secret
    of the AAD application as environment variables: AZURE_CLIENT_ID, AZURE_TENANT_ID,
    AZURE_CLIENT_SECRET. For more info about how to get the value, please see:
    https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal
"""


def main():
    client = EdgeActionsMgmtClient(
        credential=DefaultAzureCredential(),
        subscription_id="SUBSCRIPTION_ID",
    )

    response = client.edge_action_versions.begin_create(
        resource_group_name="testrg",
        edge_action_name="edgeAction1",
        version="version2",
        resource={"location": "global", "properties": {"deploymentType": "zip", "isDefaultVersion": "True"}},
    ).result()
    print(response)


# x-ms-original-file: 2025-12-01-preview/EdgeActionVersions_Create.json
if __name__ == "__main__":
    main()
