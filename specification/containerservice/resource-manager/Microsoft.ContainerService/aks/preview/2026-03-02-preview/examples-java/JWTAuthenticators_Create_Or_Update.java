...
public final class Main {
    ...
    public static void
        ...
        manager.serviceClient().getJWTAuthenticators().createOrUpdate("rg1", "clustername1", "jwt1",
            new JWTAuthenticatorInner().withProperties(new JWTAuthenticatorProperties()
                .withIssuer(new JWTAuthenticatorIssuer().withUrl("https://example.com")
                    .withAudiences(Arrays.asList("https://example.com/audience1", "https://example.com/audience2")))
                ...
                .withClaimMappings(new JWTAuthenticatorClaimMappings()
                    .withUsername(
                        new JWTAuthenticatorClaimMappingExpression().withExpression("'aks:jwt:' + claims.sub"))
                    .withGroups(new JWTAuthenticatorClaimMappingExpression()
                        .withExpression("claims.groups.split(',').map(group, 'aks:jwt:' + group)"))
                    .withExtra(Arrays.asList(new JWTAuthenticatorExtraClaimMappingExpression()
                        .withKey("fakeTokenPlaceholder").withValueExpression("claims.customfield"))))
