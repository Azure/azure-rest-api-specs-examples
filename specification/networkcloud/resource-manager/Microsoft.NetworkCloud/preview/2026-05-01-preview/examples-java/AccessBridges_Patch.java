...
public final class Main {
    ...
    public static void patchAccessBridge(com.azure.resourcemanager.networkcloud.NetworkCloudManager manager) {
        AccessBridge resource = manager.accessBridges().getByResourceGroupWithResponse("resourceGroupName",
            AccessBridgeAllowedName.BASTION, com.azure.core.util.Context.NONE).getValue();
        resource.update().withTags(mapOf("key1", "fakeTokenPlaceholder", "key2", "fakeTokenPlaceholder"))
            .withProperties(
                new AccessBridgePatchProperties().withSecurityRules(Arrays.asList(new AccessBridgeSecurityRule()
                    .withDescription("Allow management plane egress").withDirection(SecurityRuleDirection.OUTBOUND)
