...
/**
 * This sample demonstrates how to Lists the supported kubernetes versions from the underlying custom location
 *
 * @summary Lists the supported kubernetes versions from the underlying custom location
...
async function listKubernetesVersions() {
  ...
  for await (let item of client.kubernetesVersions.list(customLocationResourceUri)) {
    resArray.push(item);
  }
