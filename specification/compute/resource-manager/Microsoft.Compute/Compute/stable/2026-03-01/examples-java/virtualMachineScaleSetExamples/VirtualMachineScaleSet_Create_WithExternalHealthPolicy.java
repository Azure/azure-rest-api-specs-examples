...
public final class Main {
    /*
     * x-ms-original-file:
     * 2026-03-01/virtualMachineScaleSetExamples/VirtualMachineScaleSet_Create_WithExternalHealthPolicy.json
     */
    /**
     * Sample code: Create a scale set with external health policy enabled.
     * 
     * @param manager Entry point to ComputeManager.
     */
    public static void
        createAScaleSetWithExternalHealthPolicyEnabled(com.azure.resourcemanager.compute.ComputeManager manager) {
        manager.serviceClient().getVirtualMachineScaleSets().createOrUpdate("myResourceGroup", "{vmss-name}",
            new VirtualMachineScaleSetInner().withLocation("westus")
                ...
                .withExternalHealthPolicy(new ExternalHealthPolicy().withEnabled(true)
                    .withExpiryDuration(Duration.parse("PT10M")).withGracePeriod(Duration.parse("PT4H"))),
