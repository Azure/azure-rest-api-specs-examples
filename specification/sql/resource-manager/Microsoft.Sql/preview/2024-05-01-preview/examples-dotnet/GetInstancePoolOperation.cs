...
// get the collection of this InstancePoolOperationResource
InstancePoolOperationCollection collection = instancePool.GetInstancePoolOperations();

// invoke the operation
Guid operationId = Guid.Parse("c218773b-203f-4c7a-b174-6bd71fe20f72");
NullableResponse<InstancePoolOperationResource> response = await collection.GetIfExistsAsync(operationId);
InstancePoolOperationResource result = response.HasValue ? response.Value : null;
...
else
{
    ...
    InstancePoolOperationData resourceData = result.Data;
