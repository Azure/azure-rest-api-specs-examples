...
NullableResponse<LogicWorkflowRunOperationResource> response = await collection.GetIfExistsAsync(operationId);
LogicWorkflowRunOperationResource result = response.HasValue ? response.Value : null;
